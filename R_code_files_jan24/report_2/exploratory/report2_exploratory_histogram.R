install.packages("readxl")
library(ggplot2)
library("readxl")
data_folder <- "/Users/natasha/Documents/r_codes_stats_assignment_jan_24/Report-2-cirrhosis/exploratory_analysis"
filename <- "/messy_cirrhosis.xlsx"
file_path <- paste0(data_folder, filename)
data <- read_excel(file_path) #read.table(file_path, header = TRUE, sep ='t')
head(data)

# Reshape the data
data_long <- pivot_longer(data, cols = starts_with("under18"), names_to = "Age_Survived", values_to = "Count")

# Create a ggplot with histogram and density line
ggplot(data_long, aes(x = Count, fill = Age_Survived)) +
  geom_histogram(position = "identity", bins = 20, alpha = 0.7, color = "white") +
  geom_density(aes(y = ..count..), color = "black") +
  facet_grid(~Cirrhosis_Type + Sex, scales = "free_y") +
  labs(title = "Histogram Overlayed with Density Line",
       x = "Count",
       fill = "Survival Status") +
  theme_minimal()
